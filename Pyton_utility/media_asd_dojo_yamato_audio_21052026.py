...
CROP_PRESETS = {
    ...
    "Cover sito/news - 1200x630": {"width": 1200, "height": 630, "folder": "COVER_SITO_NEWS"},
    "Card news 800x600": {"width": 800, "height": 600, "folder": "CARD_NEWS"},
    "Galleria 4:3 - 1200x900": {"width": 1200, "height": 900, "folder": "GALLERIA_4_3"},
    "Verticale 4:5 - 1080x1350": {"width": 1080, "height": 1350, "folder": "VERTICALE_4_5"},
    "Profilo insegnante 4:5 - 1200x1500": {"width": 1200, "height": 1500, "folder": "PROFILI_4_5"},
    "A4 verticale web - 1200x1697": {"width": 1200, "height": 1697, "folder": "A4_VERTICALE_WEB"},
    "A4 verticale alta qualità - 2480x3508": {"width": 2480, "height": 3508, "folder": "A4_VERTICALE_ALTA_QUALITA"},
...
class MediaASDDojoYamatoApp:
    def __init__(self, root):
        ...
        self.video_include_audio = tk.BooleanVar(value=True)
        ...
        self.crop_selections_by_image_preset = {}
        self.crop_assigned_presets = {}
        self.crop_adjustments_by_image = {}
        self.drag_mode = None
        ...
        self.crop_preview_cache = {}
        self.crop_preview_cache_size = 1400
        self.crop_current_original_size = None
        self.crop_zoom = tk.DoubleVar(value=1.00)
        self.crop_progress_percent = tk.StringVar(value="0%")
        self.crop_brightness = tk.DoubleVar(value=1.00)
        self.crop_contrast = tk.DoubleVar(value=1.00)
        self.crop_color = tk.DoubleVar(value=1.00)
        self.crop_sharpness = tk.DoubleVar(value=1.00)
        self.crop_adjustments_by_image = {}
        self.crop_export_results = []
        self.crop_preset_vars = {}
        self.crop_report_csv_path = None
    ...
    def show_home(self):
        ...
        tk.Label(frame, text=APP_NAME, font=("Segoe UI", 28, "bold")).pack(pady=(40, 8))
        tk.Label(frame, text="Gestione media per il sito ASD Dojo Yamato", font=("Segoe UI", 13)).pack(pady=(0, 40))

        cards = tk.Frame(frame)
        cards.pack(fill="both", expand=True)

        left = tk.LabelFrame(cards, text="Convertitore media", padx=24, pady=24)
        left.pack(side="left", fill="both", expand=True, padx=(0, 18))
        tk.Label(left, text="Converti e comprimi foto e video per il sito.\n\nInclude preset web, rinomina automatica,\ndettagli conversione e cartella report.", justify="left", font=("Segoe UI", 11)).pack(fill="x", pady=(0, 24))
        tk.Button(left, text="ENTRA IN CONVERTITORE MEDIA", command=self.show_converter_section, height=3, font=("Segoe UI", 13, "bold")).pack(fill="x")

        right = tk.LabelFrame(cards, text="Crop Studio / Ritaglio immagini", padx=24, pady=24)
        right.pack(side="left", fill="both", expand=True, padx=(18, 0))
        tk.Label(right, text="Importa più immagini o una cartella.\n\nSeleziona immagini da una lista, assegna uno o più preset,\nritaglia con manina/angoli ed esporta tutto in batch.", justify="left", font=("Segoe UI", 11)).pack(fill="x", pady=(0, 24))
        tk.Button(right, text="ENTRA IN RITAGLIO IMMAGINI", command=self.show_crop_section, height=3, font=("Segoe UI", 13, "bold")).pack(fill="x")
    ...
    def _build_video_settings(self, parent):
        ...
        tk.Checkbutton(row4, text="Includi audio nell'export", variable=self.video_include_audio).pack(side="left")
    ...
    def _build_crop_page(self, parent):
        ...
        # Riga compatta superiore: punti 1-2 a sinistra, punti 3-4 a destra.
        top_row = tk.Frame(main)
        top_row.pack(fill="x", pady=(0, 6))

        left = tk.Frame(top_row)
        left.pack(side="left", fill="both", expand=True, padx=(0, 6))

        right = tk.Frame(top_row)
        right.pack(side="left", fill="both", expand=True, padx=(6, 0))

        import_frame = tk.LabelFrame(left, text="1. Import immagini", padx=6, pady=5)
        import_frame.pack(fill="x", pady=(0, 4))

        tk.Button(import_frame, text="Apri file singoli/multipli", command=self.crop_import_files).pack(side="left", fill="x", expand=True, padx=(0, 4))
        tk.Button(import_frame, text="Apri cartella immagini", command=self.crop_import_folder).pack(side="left", fill="x", expand=True, padx=(4, 0))

        list_frame = tk.LabelFrame(left, text="2. Lista immagini importate", padx=6, pady=5)
        list_frame.pack(fill="x")

        # Lista immagini con pulsanti per ogni riga:
        # nome file + ✅ OK + ❌ X. Nessuna scritta "da valutare/scartare".
        self.crop_rows_canvas = tk.Canvas(list_frame, height=165, highlightthickness=0)
        self.crop_rows_canvas.grid(row=0, column=0, sticky="nsew")

        crop_scroll_y = ttk.Scrollbar(list_frame, orient="vertical", command=self.crop_rows_canvas.yview)
        crop_scroll_y.grid(row=0, column=1, sticky="ns")
        self.crop_rows_canvas.configure(yscrollcommand=crop_scroll_y.set)

        self.crop_rows_container = tk.Frame(self.crop_rows_canvas)
        self.crop_rows_window = self.crop_rows_canvas.create_window((0, 0), window=self.crop_rows_container, anchor="nw")

        def _rows_configure(event=None):
            self.crop_rows_canvas.configure(scrollregion=self.crop_rows_canvas.bbox("all"))

        def _canvas_configure(event):
            self.crop_rows_canvas.itemconfigure(self.crop_rows_window, width=event.width)

        self.crop_rows_container.bind("<Configure>", _rows_configure)
        self.crop_rows_canvas.bind("<Configure>", _canvas_configure)

        list_frame.grid_rowconfigure(0, weight=1)
        list_frame.grid_columnconfigure(0, weight=1)

        list_buttons = tk.Frame(list_frame)
        list_buttons.grid(row=1, column=0, columnspan=2, sticky="ew", pady=(5, 0))
        tk.Button(list_buttons, text="Svuota lista", command=self.crop_clear_images).pack(side="left", fill="x", expand=True)

        preset_frame = tk.LabelFrame(right, text="3. Preset ritaglio immagine selezionata", padx=6, pady=5)
        preset_frame.pack(fill="x", pady=(0, 4))

        preset_line = tk.Frame(preset_frame)
        preset_line.pack(fill="x", pady=(0, 4))
        tk.Label(preset_line, text="Preset:").pack(side="left")
        combo = ttk.Combobox(preset_line, textvariable=self.crop_preset, values=list(CROP_PRESETS.keys()), state="readonly", width=36)
        combo.pack(side="left", fill="x", expand=True, padx=(5, 5))
        combo.bind("<<ComboboxSelected>>", lambda e: self.apply_crop_preset_and_default_box())
        tk.Button(preset_line, text="Carica griglia", command=self.apply_crop_preset_and_default_box).pack(side="left")

        tk.Button(
            preset_frame,
            text="APPLICA PRESET ALL'IMMAGINE SELEZIONATA",
            command=self.crop_apply_current_preset_to_image,
            font=("Segoe UI", 9, "bold")
        ).pack(fill="x", pady=(0, 3))

        tk.Button(
            preset_frame,
            text="Rimuovi preset dall'immagine selezionata",
            command=self.crop_remove_presets_from_current_image
        ).pack(fill="x")

        export_frame = tk.LabelFrame(right, text="4. Export immagini confermate", padx=6, pady=5)
        export_frame.pack(fill="x")

        export_opts = tk.Frame(export_frame)
        export_opts.pack(fill="x", pady=(0, 4))
        tk.Label(export_opts, text="Formato:").pack(side="left")
        ttk.Combobox(export_opts, textvariable=self.crop_output_format, values=list(CROP_OUTPUT_FORMATS.keys()), state="readonly", width=8).pack(side="left", padx=(5, 10))
        tk.Label(export_opts, text="Qualità:").pack(side="left")
        tk.Entry(export_opts, textvariable=self.crop_quality, width=6).pack(side="left", padx=(5, 0))

        folder_line = tk.Frame(export_frame)
        folder_line.pack(fill="x", pady=(0, 4))
        tk.Entry(folder_line, textvariable=self.crop_output_folder).pack(side="left", fill="x", expand=True, padx=(0, 5))
        tk.Button(folder_line, text="Cartella", command=self.choose_crop_output_folder).pack(side="left")

        tk.Button(
            export_frame,
            text="ESPORTA TUTTE LE IMMAGINI CON PRESET",
            command=self.crop_export_all,
            font=("Segoe UI", 9, "bold")
        ).pack(fill="x", pady=(0, 4))

        progress_line = tk.Frame(export_frame)
        progress_line.pack(fill="x")
        self.crop_progress = ttk.Progressbar(progress_line, orient="horizontal", mode="determinate")
        self.crop_progress.pack(side="left", fill="x", expand=True)
        tk.Label(progress_line, textvariable=self.crop_progress_percent, width=6).pack(side="left", padx=(5, 0))

        # Area centrale: metà anteprima, metà regolazioni immagine.
        preview_main = tk.PanedWindow(main, orient="horizontal", sashwidth=8, sashrelief="raised")
        preview_main.pack(fill="both", expand=True, pady=(0, 6))

        canvas_frame = tk.LabelFrame(preview_main, text="5. Anteprima immagine e ritaglio", padx=6, pady=5)
        controls_side = tk.LabelFrame(preview_main, text="6. Regolazioni SOLO immagine selezionata", padx=8, pady=8)

        preview_main.add(canvas_frame, minsize=480)
        preview_main.add(controls_side, minsize=420)

        top_canvas_controls = tk.Frame(canvas_frame)
        top_canvas_controls.pack(fill="x", pady=(0, 4))

        tk.Label(
            top_canvas_controls,
            text="Riquadro verde: trascina il centro per spostare, trascina gli angoli per ridimensionare.",
            fg="#444444"
        ).pack(side="left", padx=(0, 12))

        tk.Checkbutton(top_canvas_controls, text="Blocca proporzione", variable=self.lock_ratio).pack(side="left")
        tk.Checkbutton(top_canvas_controls, text="Mostra griglia", variable=self.show_grid, command=self.redraw_crop_overlay).pack(side="left", padx=(8, 0))

        tk.Button(top_canvas_controls, text="Zoom +", command=self.crop_zoom_in).pack(side="left", padx=(12, 3))
        tk.Button(top_canvas_controls, text="Zoom -", command=self.crop_zoom_out).pack(side="left", padx=(3, 3))
        tk.Button(top_canvas_controls, text="Reset zoom", command=self.crop_zoom_reset).pack(side="left", padx=(3, 0))

        self.crop_canvas = tk.Canvas(canvas_frame, bg="#222222", cursor="crosshair", height=360)
        ...
        tk.Label(
            controls_side,
            text="Regola solo l\'immagine selezionata. Le modifiche vengono salvate per questa immagine e applicate al suo export.",
            justify="left",
            fg="#444444"
        ).pack(anchor="w", pady=(0, 8))

        def add_adj_slider_horizontal(parent, label, variable):
            row = tk.Frame(parent)
            row.pack(fill="x", pady=(0, 8))

            tk.Label(row, text=label, width=12, anchor="w", font=("Segoe UI", 9, "bold")).pack(side="left")
            ...
            scale = tk.Scale(
                ...
                resolution=0.05,
                orient="horizontal",
                command=self.refresh_crop_preview_adjustments,
                length=260
            )
            scale.pack(side="left", fill="x", expand=True, padx=(6, 6))

            tk.Label(row, textvariable=variable, width=5).pack(side="left")
            return scale

        add_adj_slider_horizontal(controls_side, "Luminosità", self.crop_brightness)
        add_adj_slider_horizontal(controls_side, "Contrasto", self.crop_contrast)
        add_adj_slider_horizontal(controls_side, "Colore", self.crop_color)
        add_adj_slider_horizontal(controls_side, "Nitidezza", self.crop_sharpness)

        tk.Button(
            controls_side,
            text="Reset regolazioni immagine",
            command=self.reset_crop_adjustments
        ).pack(fill="x", pady=(4, 8))

        info_box = tk.LabelFrame(controls_side, text="Stato immagine selezionata", padx=6, pady=6)
        info_box.pack(fill="x", pady=(4, 0))
        tk.Label(info_box, textvariable=self.crop_status, anchor="w", justify="left").pack(fill="x")

        export_box = tk.LabelFrame(controls_side, text="Stato export", padx=6, pady=6)
        export_box.pack(fill="x", pady=(8, 0))
        tk.Label(export_box, textvariable=self.crop_export_status, anchor="w", justify="left").pack(fill="x")

        # Ripristino aree riepilogo: immagini confermate + report esportazione.
        bottom_row = tk.Frame(main)
        bottom_row.pack(fill="x", pady=(0, 4))

        queue_frame = tk.LabelFrame(bottom_row, text="7. Immagini confermate per export", padx=6, pady=5)
        queue_frame.pack(side="left", fill="both", expand=True, padx=(0, 5))
        ...
        self.crop_queue_tree.heading("presets", text="Preset applicati")
        self.crop_queue_tree.column("image", width=260)
        self.crop_queue_tree.column("presets", width=520)
        self.crop_queue_tree.pack(fill="x")

        report_frame = tk.LabelFrame(bottom_row, text="8. Report immagini esportate", padx=6, pady=5)
        report_frame.pack(side="left", fill="both", expand=True, padx=(5, 0))

        report_cols = ("image", "preset", "output", "kb", "status")
        self.crop_report_tree = ttk.Treeview(report_frame, columns=report_cols, show="headings", height=5)
        self.crop_report_tree.heading("image", text="Immagine")
        self.crop_report_tree.heading("preset", text="Preset")
        self.crop_report_tree.heading("output", text="Output")
        self.crop_report_tree.heading("kb", text="KB")
        self.crop_report_tree.heading("status", text="Stato")
        self.crop_report_tree.column("image", width=160)
        self.crop_report_tree.column("preset", width=150)
        self.crop_report_tree.column("output", width=360)
        self.crop_report_tree.column("kb", width=60, anchor="e")
        self.crop_report_tree.column("status", width=70, anchor="center")
        self.crop_report_tree.pack(fill="x")

        tk.Button(report_frame, text="Svuota report", command=self.crop_clear_report).pack(anchor="e", pady=(4, 0))

        self.root.bind("<Up>", self.crop_select_previous_image)
        self.root.bind("<Down>", self.crop_select_next_image)
        self.crop_refresh_listbox()
        self.crop_refresh_queue()

    def crop_confirm_selected_image(self):
        """Conferma l'immagine selezionata applicando il preset attualmente scelto."""
        if self.crop_current_index is None or self.crop_current_index >= len(self.crop_images) or not self.crop_image_path:
            messagebox.showwarning("Nessuna immagine selezionata", "Seleziona una immagine dalla lista.")
            return
        self.crop_apply_current_preset_to_image()

    def crop_remove_selected_images(self):
        if self.crop_current_index is None:
            messagebox.showwarning("Nessuna immagine selezionata", "Seleziona una immagine da scartare.")
            return
        self.crop_remove_image_by_index(self.crop_current_index)

    def crop_apply_current_preset_to_image(self):
        if self.crop_current_index is None or self.crop_current_index >= len(self.crop_images) or not self.crop_image_path:
            messagebox.showwarning("Nessuna immagine selezionata", "Seleziona una immagine dalla lista.")
            return
        ...
        self.crop_save_current_adjustments()
        preset_name = self.crop_preset.get()
        path_key = str(self.crop_image_path)
        assigned = self.crop_assigned_presets.setdefault(path_key, set())
        assigned.add(preset_name)
        key = self.crop_current_selection_key(self.crop_image_path, preset_name)
        if key:
            self.crop_selections_by_image_preset[key] = self.crop_selection
        self.crop_refresh_listbox()
        self.crop_refresh_queue()
        self.crop_status.set(f"✅ Confermata: {self.crop_image_path.name} con preset {preset_name}")

    def crop_remove_presets_from_current_image(self):
        if self.crop_current_index is None or self.crop_current_index >= len(self.crop_images) or not self.crop_image_path:
            messagebox.showwarning("Nessuna immagine selezionata", "Seleziona una immagine dalla lista.")
            return
        path_key = str(self.crop_image_path)
        self.crop_assigned_presets[path_key] = set()
        keys_to_remove = [k for k in self.crop_selections_by_image_preset if k.startswith(path_key + "||")]
        for key in keys_to_remove:
            self.crop_selections_by_image_preset.pop(key, None)
        self.crop_refresh_listbox()
        self.crop_refresh_queue()
        self.crop_status.set(f"Preset rimossi da {self.crop_image_path.name}")
    ...
    def crop_refresh_listbox(self):
        """Aggiorna la lista immagini con pulsanti OK/X per ogni riga."""
        if not hasattr(self, "crop_rows_container"):
            return

        for widget in self.crop_rows_container.winfo_children():
            widget.destroy()

        for idx, path in enumerate(self.crop_images):
            row = tk.Frame(self.crop_rows_container, pady=0)
            row.pack(fill="x", expand=True)

            is_selected = idx == self.crop_current_index
            bg = "#d9ecff" if is_selected else "#ffffff"
            row.configure(bg=bg)

            number = f"{idx+1:02d}."
            presets = sorted(self.crop_assigned_presets.get(str(path), set()))
            preset_text = f"  | Preset: {', '.join(presets)}" if presets else ""

            label = tk.Label(
                row,
                text=f"{number} {path.name}{preset_text}",
                anchor="w",
                bg=bg,
                fg="#000000",
                padx=2
            ...
            label.bind("<Button-1>", lambda event, i=idx: self.crop_select_image_index(i))

            tk.Button(
                row,
                text="✓ OK",
                command=lambda i=idx: self.crop_confirm_image_by_index(i),
                bg="#1f9d55",
                fg="white",
            ...
            tk.Button(
                row,
                text="×",
                command=lambda i=idx: self.crop_remove_image_by_index(i),
                bg="#dc3545",
                fg="white",
        ...
        if hasattr(self, "crop_rows_canvas"):
            self.crop_rows_canvas.configure(scrollregion=self.crop_rows_canvas.bbox("all"))

    def crop_select_previous_image(self, event=None):
        if not self.crop_images:
            return
        if self.crop_current_index is None:
            self.crop_select_image_index(0)
        else:
            self.crop_select_image_index(max(0, self.crop_current_index - 1))

    def crop_select_next_image(self, event=None):
        if not self.crop_images:
            return
        if self.crop_current_index is None:
            self.crop_select_image_index(0)
        else:
            self.crop_select_image_index(min(len(self.crop_images) - 1, self.crop_current_index + 1))

    def get_fast_preview_source(self, path: Path):
        """Carica una preview ridotta per rendere veloce il passaggio tra immagini."""
        key = str(path)
        if key in self.crop_preview_cache:
            return self.crop_preview_cache[key].copy()

        with Image.open(path) as img:
            img = img.convert("RGB")
            img.thumbnail((self.crop_preview_cache_size, self.crop_preview_cache_size), Image.Resampling.LANCZOS)
            preview = img.copy()

        # Cache semplice: evita di tenere troppe immagini in memoria.
        if len(self.crop_preview_cache) > 30:
            self.crop_preview_cache.clear()
        self.crop_preview_cache[key] = preview
        return preview.copy()

    def crop_select_image_index(self, index):
        """Seleziona una immagine dalla lista a righe."""
        if index < 0 or index >= len(self.crop_images):
            return
        self.crop_save_current_selection()
        self.crop_save_current_adjustments()
        self.crop_current_index = index
        self.crop_load_current_image()
        self.crop_refresh_listbox()

    def crop_confirm_image_by_index(self, index):
        """Conferma la riga cliccata applicando il preset attuale."""
        self.crop_select_image_index(index)
        self.crop_confirm_selected_image()

    def crop_remove_image_by_index(self, index):
        """Scarta direttamente la riga cliccata."""
        if index < 0 or index >= len(self.crop_images):
            return
        path = self.crop_images[index]
        path_key = str(path)
        self.crop_assigned_presets.pop(path_key, None)
        self.crop_selections_by_image.pop(path_key, None)
        self.crop_adjustments_by_image.pop(path_key, None)
        keys_to_remove = [k for k in self.crop_selections_by_image_preset if k.startswith(path_key + "||")]
        for key in keys_to_remove:
            self.crop_selections_by_image_preset.pop(key, None)
        del self.crop_images[index]
        ...
        if not self.crop_images:
            ...
            self.crop_current_index = min(index, len(self.crop_images) - 1)
            self.crop_load_current_image()

        self.crop_refresh_listbox()
        self.crop_refresh_queue()
        self.crop_status.set(f"❌ Scartata dalla lista: {path.name}")

    def crop_on_select_image(self, event=None):
        # Legacy callback, kept for compatibility.
        return

    def crop_current_selection_key(self, path=None, preset_name=None):
        if path is None:
            path = self.crop_image_path
        if preset_name is None:
            preset_name = self.crop_preset.get()
        if not path:
            return None
        return f"{str(path)}||{preset_name}"

    def crop_get_selection_for(self, path, preset_name):
        key = self.crop_current_selection_key(path, preset_name)
        if key and key in self.crop_selections_by_image_preset:
            return self.crop_selections_by_image_preset[key]
        return self.crop_selections_by_image.get(str(path))

    def crop_set_selection_for_current(self):
        if self.crop_image_path and self.crop_selection:
            key = self.crop_current_selection_key()
            if key:
                self.crop_selections_by_image_preset[key] = self.crop_selection
            self.crop_selections_by_image[str(self.crop_image_path)] = self.crop_selection

    def crop_save_current_selection(self):
        self.crop_set_selection_for_current()
    ...
    def crop_load_current_image(self):
        ...
        try:
            ...
            # Anteprima veloce: usiamo una copia ridotta per il canvas.
            # L'export continua a usare il file originale ad alta risoluzione.
            with Image.open(path) as original_info:
                original_w, original_h = original_info.size
            self.crop_current_original_size = (original_w, original_h)
            self.crop_original = self.get_fast_preview_source(path)
            self.crop_selection = self.crop_get_selection_for(path, self.crop_preset.get())
            self.crop_load_adjustments_for_image(path)
            self.crop_status.set(f"Immagine selezionata: {path.name} - originale {original_w}x{original_h} px")
            if not self.crop_output_folder.get():
    ...
    def crop_default_selection_for_preset(self, image_size, preset_name):
    ...
    def crop_ensure_selections_for_assigned_presets(self, path, presets):
        try:
            preview_img = self.get_fast_preview_source(path)
            image_size = preview_img.size
            for preset_name in presets:
                key = self.crop_current_selection_key(path, preset_name)
                if key and key not in self.crop_selections_by_image_preset:
                    self.crop_selections_by_image_preset[key] = self.crop_default_selection_for_preset(image_size, preset_name)
        except Exception:
            pass
    ...
    def crop_update_current_preset_assignment(self):
        ...
        self.crop_ensure_selections_for_assigned_presets(self.crop_image_path, assigned)
        self.crop_refresh_listbox()
        self.crop_refresh_queue()
        self.crop_status.set(f"Preset assegnati a {self.crop_image_path.name}: {', '.join(sorted(assigned)) if assigned else 'nessuno'}")
    ...
    def crop_apply_selected_presets_to_all(self):
        ...
        for path in self.crop_images:
            ...
            self.crop_ensure_selections_for_assigned_presets(path, selected)
        self.crop_refresh_listbox()
        self.crop_refresh_queue()
    ...
    def apply_crop_preset_and_default_box(self):
        ...
        if self.crop_image_path:
            saved = self.crop_get_selection_for(self.crop_image_path, self.crop_preset.get())
            if saved:
                self.crop_selection = saved
    ...
    def crop_current_adjustments_tuple(self):
        return (
            float(self.crop_brightness.get()),
            float(self.crop_contrast.get()),
            float(self.crop_color.get()),
            float(self.crop_sharpness.get())
        )

    def crop_save_current_adjustments(self):
        """Salva le regolazioni solo per l'immagine attualmente selezionata."""
        if self.crop_image_path:
            self.crop_adjustments_by_image[str(self.crop_image_path)] = self.crop_current_adjustments_tuple()

    def crop_load_adjustments_for_image(self, path):
        """Carica le regolazioni specifiche della singola immagine."""
        values = self.crop_adjustments_by_image.get(str(path), (1.00, 1.00, 1.00, 1.00))
        self.crop_brightness.set(values[0])
        self.crop_contrast.set(values[1])
        self.crop_color.set(values[2])
        self.crop_sharpness.set(values[3])

    def crop_get_adjustments_for_path(self, path):
        return self.crop_adjustments_by_image.get(str(path), (1.00, 1.00, 1.00, 1.00))

    def apply_adjustment_values_to_image(self, image, values):
        try:
            image = ImageEnhance.Brightness(image).enhance(float(values[0]))
            image = ImageEnhance.Contrast(image).enhance(float(values[1]))
            image = ImageEnhance.Color(image).enhance(float(values[2]))
            image = ImageEnhance.Sharpness(image).enhance(float(values[3]))
        except Exception:
            pass
        return image

    def apply_crop_adjustments_to_image(self, image):
        """Applica le regolazioni dell'immagine attualmente selezionata."""
        return self.apply_adjustment_values_to_image(image, self.crop_current_adjustments_tuple())

    def reset_crop_adjustments(self):
        self.crop_brightness.set(1.00)
        self.crop_contrast.set(1.00)
        self.crop_color.set(1.00)
        self.crop_sharpness.set(1.00)
        self.crop_save_current_adjustments()
        self.display_crop_image()
        if self.crop_image_path:
            self.crop_status.set(f"Regolazioni ripristinate solo per: {self.crop_image_path.name}")
        else:
            self.crop_status.set("Regolazioni immagine ripristinate.")

    def refresh_crop_preview_adjustments(self, event=None):
        self.crop_save_current_adjustments()
        self.display_crop_image()

    def crop_zoom_in(self):
        self.crop_zoom.set(min(3.0, round(float(self.crop_zoom.get()) + 0.25, 2)))
        self.display_crop_image()
        self.crop_status.set(f"Zoom anteprima: {self.crop_zoom.get():.2f}x")

    def crop_zoom_out(self):
        self.crop_zoom.set(max(0.5, round(float(self.crop_zoom.get()) - 0.25, 2)))
        self.display_crop_image()
        self.crop_status.set(f"Zoom anteprima: {self.crop_zoom.get():.2f}x")

    def crop_zoom_reset(self):
        self.crop_zoom.set(1.00)
        self.display_crop_image()
        self.crop_status.set("Zoom anteprima ripristinato.")
    ...
    def display_crop_image(self):
        ...
        scale = min(canvas_w / img_w, canvas_h / img_h) * float(self.crop_zoom.get())
        ...
        preview_source = self.apply_crop_adjustments_to_image(self.crop_original.copy())
        self.crop_preview_image = preview_source.resize((preview_w, preview_h), Image.Resampling.LANCZOS)
    ...
    def crop_clear_report(self):
        self.crop_export_results = []
        self.crop_report_csv_path = None
        if hasattr(self, "crop_report_tree"):
            for item in self.crop_report_tree.get_children():
                self.crop_report_tree.delete(item)
        self.crop_export_status.set("Report svuotato.")

    def crop_add_report_row(self, row):
        self.crop_export_results.append(row)
        if hasattr(self, "crop_report_tree"):
            self.crop_report_tree.insert(
                "",
                "end",
                values=(
                    row.get("image", ""),
    ...
    def crop_write_report_csv(self, output_root: Path):
        report_folder = output_root / "REPORT_EXPORT"
        report_folder.mkdir(parents=True, exist_ok=True)
        self.crop_report_csv_path = report_folder / "report_ritagli_export.csv"
        with open(self.crop_report_csv_path, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.writer(f, delimiter=";")
            writer.writerow(["Immagine", "Preset", "Output", "KB", "Stato", "Errore"])
            for row in self.crop_export_results:
                writer.writerow([
                    row.get("image", ""),
                    row.get("preset", ""),
                    row.get("output", ""),
    ...
    def crop_export_current(self):
        ...
        presets = sorted(self.crop_assigned_presets.get(str(path), set()))
        if not presets:
            messagebox.showwarning("Nessun preset", "Assegna almeno un preset all'immagine selezionata.")
            return
        tasks = [(path, preset_name) for preset_name in presets]
    ...
    def crop_export_tasks(self, tasks):
        output_root = Path(self.crop_output_folder.get()) / "PRESET_PER_SITO_WEB"
        output_root.mkdir(parents=True, exist_ok=True)
        self.crop_export_results = []
        if hasattr(self, "crop_report_tree"):
            self.root.after(0, self.crop_clear_report)
        ...
        for idx, (path, preset_name) in enumerate(tasks, start=1):
            row = {"image": path.name, "preset": preset_name, "output": "", "kb": "", "status": "OK", "error": ""}
            try:
                output_path = self.crop_export_single(path, preset_name, output_root)
                row["output"] = str(output_path)
                row["kb"] = f"{file_size_kb(output_path):.1f}"
                exported += 1
            except Exception as exc:
                row["status"] = "ERRORE"
                row["error"] = str(exc)
                errors.append(f"{path.name} / {preset_name}: {exc}")
            self.root.after(0, lambda r=row: self.crop_add_report_row(r))
        ...
        self.crop_write_report_csv(output_root)
        msg = f"Export completati: {exported}/{len(tasks)}\nCartella unica export:\n{output_root}\nReport:\n{self.crop_report_csv_path}"
        if errors:
            msg += "\n\nErrori:\n" + "\n".join(errors[:10])
        ...
        self.root.after(0, lambda: self.crop_export_status.set(f"Export completato: {exported}/{len(tasks)} - report CSV creato."))
    ...
    def crop_export_single(self, path: Path, preset_name: str, output_root: Path):
        ...
        selection = self.crop_get_selection_for(path, preset_name)
        with Image.open(path) as original:
            ...
            # Le coordinate salvate sono sull'anteprima ridotta.
            # Qui le convertiamo sul file originale ad alta risoluzione.
            preview_ref = self.get_fast_preview_source(path)
            preview_w, preview_h = preview_ref.size
            original_w, original_h = original.size
            scale_x = original_w / max(1, preview_w)
            scale_y = original_h / max(1, preview_h)
            ox1 = int(x1 * scale_x)
            oy1 = int(y1 * scale_y)
            ox2 = int(x2 * scale_x)
            oy2 = int(y2 * scale_y)
            ...
            if (ox2 - ox1) < 2 or (oy2 - oy1) < 2:
                selection = self.crop_default_selection_for_preset(original.size, preset_name)
                ox1, oy1, ox2, oy2 = selection

            # Taglia ESATTAMENTE dentro i bordi del riquadro verde, riportato sul file originale.
            cropped = original.crop((ox1, oy1, ox2, oy2)).resize((preset["width"], preset["height"]), Image.Resampling.LANCZOS)
    ...
    def convert_video(self, file_path, output_folder, image_counter, video_counter):
        ...
        # AUDIO DI DEFAULT ATTIVO.
        # La mappa con ? evita errore se il file originale non ha audio.
        # aresample aiuta a mantenere audio sincronizzato/corretto su MOV/iPhone.
        if self.video_include_audio.get():
            cmd.extend([
                "-map", "0:a:0?",
                "-c:a", "aac",
                "-b:a", audio_bitrate,
