...
type TheoryResource = {
  id: string
  section: 'fondamenti' | 'risorse'
  category: string | null
  title: string
  description: string | null
  resource_type: 'file' | 'video' | 'youtube' | 'social' | 'link'
...
function AdminTeoria() {
  ...
  const [section, setSection] = useState<'fondamenti' | 'risorse'>('fondamenti')
  const [category, setCategory] = useState('KATA')
  const [title, setTitle] = useState('')
  const [description, setDescription] = useState('')
  const [resourceType, setResourceType] = useState<'file' | 'video' | 'youtube' | 'social' | 'link'>('file')
  ...
  const filteredResources = useMemo(() => {
    return resources.filter((item) => item.section === section)
  }, [resources, section])
  ...
  function getResourceTypeLabel(type: TheoryResource['resource_type']) {
  ...
  return (
    <div>
      ...
      <div style={adminCardStyle}>
        <h3>{editingId ? 'Modifica contenuto Teoria' : 'Inserisci contenuto Teoria'}</h3>

        <form onSubmit={handleSaveResource} style={formStyle}>
          <div style={twoColumnsStyle}>
            <select
              value={section}
              onChange={(e) => setSection(e.target.value as 'fondamenti' | 'risorse')}
            >
          ...
          <div style={twoColumnsStyle}>
            <select
              value={resourceType}
              onChange={(e) =>
                setResourceType(e.target.value as 'file' | 'video' | 'youtube' | 'social' | 'link')
              }
            >
              <option value="file">Documento / PDF / File</option>
          ...
          {(resourceType === 'file' || resourceType === 'video') && (
            <div style={{ display: 'grid', gap: '8px' }}>
              ...
              <input
                key={fileInputKey}
                type="file"
                accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,video/mp4,video/webm,video/quicktime"
                onChange={(e) => setResourceFile(e.target.files?.[0] ?? null)}
              />

              {resourceFile && (
                <small style={mutedText}>File selezionato: {resourceFile.name}</small>
              )}
            </div>
          )}
...
const adminListHeaderStyle: CSSProperties = {
  display: 'flex',
